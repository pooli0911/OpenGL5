// 
// Example 3LM : Light Map 運作原理展示
// 
// 地板 g_pFloor，使用 ShadingMode::PHONG_SHADING
// 本範例使用  setTiling(10,10) 將 diffuse 貼圖座標從 (0,0)-(1,1)，變成 (0,0)-(10,10)
//			   setLightMapTiling(2, 2)則是設定 light map 貼圖座標從 (0,0)-(1,1)，變成 (0,0)-(2,2)
// 
// 三個告示板都使用預設的 diffuse 貼圖，所以無需另外執行 setTextureLayer
// 每一張貼圖都帶有 alpha (RGBA) 的 PNG 圖檔。也可以使用 ShadingMode::PHONG_SHADING
// 
// Light Map 使用 vsPerPixelLighting.glsl fsPerPixelLighting.glsl 這組，
//	   設定使用 Light Map，透過 setTextureLayer(TextureMode::DIFFUSE_MAP | TextureMode::LIGHT_MAP)
//
